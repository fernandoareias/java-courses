...
import com.springbatch.migracaodados.config.ArquivoPartitioner;
...
public class ArquivoPessoaReaderConfig {
	...
	@Autowired
	private ArquivoPartitioner arquivoPartitioner;

	@StepScope
	@Bean
	public CustomARquivoReader<Pessoa> arquivoPessoaReader(@Value("#{stepExecutitionContext['particao']}") Integer particao){
		return new CustomARquivoReader<Pessoa>(
				arquivoPessoaReader(arquivoPartitioner.calcularPrimeiroItemLeitura(particao)),
				arquivoPartitioner.getItensLimit()
		);
